<template>
    <main>
        <div class="panel-navigation">
            <button :class="{ 'active': panel_access}" class="btn-left-bordered" @click="panel_access = true, panel_invite = false, panel_searchs = false">
                Gerencimento usuários
            </button>
            <button :class="{ 'active': panel_invite}" @click="panel_invite = true, panel_access = false, panel_searchs = false">
                Códigos de convite
            </button>
            <button :class="{ 'active': panel_searchs}" class="btn-right-bordered" @click="panel_searchs = true, panel_access = false, panel_invite = false">
                Buscas
            </button>
        </div>
        <div id="container-access" class="div-container" v-show="panel_access">
            <div class="div-panel">
                ...
                <div class="textfield-buttons textfield-buttons-access">
                    <button class="btn btn-success" type="submit"  @click="apiCall('add_admin')">Promover administrador</button>
                    <button class="btn btn-danger" type="submit"  @click="apiCall('rmv_admin')">Revogar administrador</button>
                </div>
                <div class="textfield-buttons textfield-buttons-access">
                    <button class="btn btn-danger" type="submit"  @click="apiCall('delete')">Deletar usuário</button>
                </div>
                <div class="textfield-buttons textfield-buttons-access" @click="apiCall('block')">
                    <button class="btn btn-warning input" type="submit">Bloquear usuário</button>
                </div>
            </div>
        </div>
            ...
            <div id="div-panel-invite" class="div-panel">
                ...
                <div class="container-subpanel-invite">
                    <div class="textfield-buttons textfield-buttons-invites">
                        <button id="btn-create-invites" :class="{ 'active': panel_invite_create}" class="button-std btn-left-bordered" @click="panel_invite_create = true, panel_invite_search = false">
                            Criar convite
                        </button>
                        <button id="btn-search-invites" :class="{ 'active': panel_invite_search}" class="button-std btn-right-bordered" @click="panel_invite_search = true, panel_invite_create = false">
                            Buscar convites criados
                        </button>
                    </div>
                    <div class="panel-create-invite" v-show="panel_invite_create && !panel_invite_search">
                        <div class="code-invite">
                            <h3>{{ invite_created }}</h3>
                        </div>
                        <button class="btn btn-success" type="submit"  @click="createInvite">Criar convite</button>
                    </div>
                    <div class="panel-create-invite" v-show="!panel_invite_create && panel_invite_search">
                        <div class="div-invites-search" v-for="invite in invites" :key="invite.id">
                            <h3>{{ invite.code }}</h3>
                            <div class="flex"></div>
...
export default defineComponent({
    ...
    methods: {
        apiCall(type: string) {
            ...
            this.customAlert.customAlert("", phrase1+" do usuário ?", true, "")
            .then(result => {
                if(result.isConfirmed) {
                    this.customAlert.inputConfirm("Insira a sua senha", "Senha", "Insira a senha...")
                    .then(result => {
                        if(!result.dismiss){
                            //INSERT API CODE
